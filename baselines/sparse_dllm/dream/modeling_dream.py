...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids=None, unsqueeze_dim=1):
    ...
    # query_len, key_len = position_offset + q.shape[-2], position_offset + k.shape[-2]
...
class DreamSdpaAttention(DreamAttention):
    ...
    def forward(
        ...
        if cache_state == 1:
            ...
            customcache.filter_cache(self.layer_idx, query_states[:, :, position_offset: position_offset + self.config.block_len, :], position_offset, self.config.block_len)
        elif cache_state == 2:
...
class DreamBaseModel(DreamPreTrainedModel):
    ...
    def forward(
        ...
        if cache_state != 2:
            ...
            position_ids = torch.arange(position_offset, position_offset + self.block_len, device=hidden_states.device).unsqueeze(0)
