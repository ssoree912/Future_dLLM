#!/usr/bin/env bash
# Run every Future_dLLM eval dataset except the multiple-choice ones through dLLM-Cache's decoding,
# one after another on a single GPU.
#
#   scripts/run_all_dllmcache_dream.sh [method]
#   DATASETS="gsm8k humaneval" scripts/run_all_dllmcache_dream.sh dllm_cache
#
# Each dataset is retried a few times: this box drops CUDA initialisation now and
# then, and run_eval_dllmcache_dream.sh keeps a resume store, so a retry picks up where
...
SUMMARY="$REPO/logs/eval/dllmcache_dream_all_${METHOD}_$(date +%Y%m%d_%H%M%S).txt"
...
for dataset in $DATASETS; do
  ...
  for attempt in $(seq 1 "$RETRIES"); do
    ...
    if bash "$REPO/scripts/run_eval_dllmcache_dream.sh" "$dataset" "$run_method"; then
      status=ok
      break
    fi
