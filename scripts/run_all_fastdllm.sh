#!/usr/bin/env bash
# Run every Future_dLLM eval dataset except MMLU through Fast-dLLM v1's decoding,
# one after another on a single GPU.
#
#   scripts/run_all_fastdllm.sh [method]
#   DATASETS="gsm8k humaneval" scripts/run_all_fastdllm.sh dual_cache_parallel
#
# Each dataset is retried a few times: this box drops CUDA initialisation now and
# then, and run_eval_fastdllm.sh keeps a resume store, so a retry picks up where
# the crash left off instead of starting over.
#
# arc_c / piqa / gpqa are scored by diffusion likelihood, which never generates -
# the cache and parallel-decoding knobs do nothing there, so they run under the
# "baseline" name whatever method is asked for.
set -uo pipefail
...
DEFAULT_DATASETS="gsm8k humaneval math500 \
...
SUMMARY="$REPO/logs/eval/fastdllm_all_${METHOD}_$(date +%Y%m%d_%H%M%S).txt"
...
for dataset in $DATASETS; do
  ...
  for attempt in $(seq 1 "$RETRIES"); do
    ...
    if bash "$REPO/scripts/run_eval_fastdllm.sh" "$dataset" "$run_method"; then
      status=ok
      break
    fi
