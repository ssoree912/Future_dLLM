...
#
# Labels under teacher_sparse_* describe blocks Sparse-dLLM's own decode
# produced, so --decode sparse replays the selection-time forward through their
# model. Training on our loop's states and deploying in theirs is exactly the
# train/deploy mismatch this pipeline exists to avoid.
set -euo pipefail
...
RUN_NAME="${RUN_NAME:-sparse_5ds_500-371-150-100-500_e${EPOCHS}_lr${LR}_len${MAX_SEQ_LEN}_${RUN_TAG}}"
...
printf 'student training on Sparse-dLLM decode labels\nmodel=%s\nteacher=%s\nmax_seq_len=%s\ngpu=%s\nrun=%s\nlog=%s\n' \
...
echo "student training on Sparse-dLLM decode labels complete"
